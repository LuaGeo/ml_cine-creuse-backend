...
def get_similar_movies(title_id, n=20):
    similar = recommendations_lookup.get(str(title_id), [])
    return similar[:n]

# Code anciennement dans precompute_recommendations.py, déplacé ici pour éviter de recalculer à chaque lancement de l'appplication. Le pré-calcul est fait une fois, et les résultats sont stockés dans un JSON pour un accès rapide :
